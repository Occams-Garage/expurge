...
## Decisions / outcomes
- Default is ephemeral. This is a reversal of the initial spec: persistence is opt-in, not opt-out-of-delete.
...
- Run-state scope, reconciled: ephemeral ≠ run state can vanish mid-session. MV3 event page spin-down is always survived (session-scoped run state, in storage). Browser close is survived only under the profile-storage opt-in. Ephemeral users get a session-scoped run; opt-in users get cross-session resume.
- One intentional data-exit point, disclosed proactively: data leaves the device only when the user sends an opt-out request (that's the purpose, not a leak). Stated plainly, not discovered. "Store locally" must never become "send us."
- Run metadata is never transmitted, regardless of toggle.
- Re-verification signals depend on opt-in: the in-the-wild drift signal (from observed `load_error`/`challenge` skips) only comes from opt-in-metadata users who choose to report it. No silent telemetry, ever.
- Export/import stays a v1 feature, but now applies to opt-in users only (ephemeral default has nothing to export).
