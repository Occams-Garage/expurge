...
## Decisions / outcomes
- Broker table in Run section rebuilds from scratch every 2s during active run. Diffing deferred: ≤25 broker rows, no interactive elements inside table rows, DOM replacement has no UX consequence.
- Poll re-renders Results section if visible, collapsing open draft panels. Acceptable edge case: expected flow is run → complete → view results; polling stops on completion so panels are only at risk during a small mid-run window.
