...
### Q-009 resolution
Q-009 asked whether `browser.permissions.request()` requires a user gesture in Firefox 140+. Answer: yes, it requires a user gesture. Resolution is by design: the Start button click is the user gesture, so the handler can call `permissions.request()` immediately. No workaround or deferred call is needed. See also M0–M3 implementation where the popup's Start button already uses this pattern.
