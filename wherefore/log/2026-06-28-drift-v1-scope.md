...
## Decisions / outcomes
- **v1: one trigger only** — time-based expiry. Self-contained; no report mechanism needed; keeps the dataset honest at 25 sites.
- **v2: failure trigger** — observed skips (`load_error` / `challenge`) become reportable via a user-initiated report button. Activates alongside the trusted-verifier tier when re-verification volume grows.
