...
- Below the three send surfaces (mailto / .eml / copy-paste), a "Mark as sent" button records an `opted_out_at` timestamp on the work item.
...
- Rationale: users should know they're sending to a general inbox before they compose and send, not discover it afterward when there's no response.
