...
## Decisions / outcomes
- Ship eight sidebar views, all pure-derived by `deriveView`: the design's six plus `stopped` and `offsite`.
- `stopped` view: honest "checked X of Y", excludes abandoned `run_stopped` items; distinct from `done`'s "all clear".
- `offsite` view: no verdict controls when the broker tab wanders off-host, so a listing can't be "confirmed" on google.com. Gated on `isOnHost`, which also catches a lookalike host sitting at the results pathname.
...
- Self-hosted the fonts: dropped the Google Fonts CDN across all surfaces.
- Removed the now-dead `scripting` and `webNavigation` permissions.
- Sidebar UX: `open_at_install:false` (no auto-open on load) + a "Show scan panel" re-open button.
- Q-015 empirically confirmed (double gesture works in Firefox 140).
