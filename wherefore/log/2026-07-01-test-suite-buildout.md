...
unbuilt code. Reconciled it: test the shipped M0 to M6 code now, and reserve the dataset-verify
...
## Decisions / outcomes
- Scope = shipped code (M0 to M6). The plan's "crown jewels" (`verifyAndLoadDataset`, schema
  validator, `expurge-data`) don't exist yet → RESERVED (M7/M9), built test-first with the
  feature.
- Today's crown-jewel-rigor target is the draft gate (`evaluateGate`) plus opt-out
...
- The draft plan was written against the app AS DESIGNED; an audit found no `crypto.subtle`,
  no `fetch()`, no `brokers.json`, no `.github/`: only M0 to M6 ships. Testing phantom modules is
